...
(function () {
    ...
    // Converts MTA daytime_routes string (e.g. "N R W") to standard trunk colors
    function mtaColor(routes) {
        if (!routes) return null;
        const r = routes.trim()[0].toUpperCase();
        if ("123".includes(r)) return "#ee352e"; // Red
        if ("456".includes(r)) return "#00933c"; // Green
        if ("7".includes(r))   return "#b933ad"; // Purple
        if ("ACE".includes(r)) return "#0039a6"; // Blue
        if ("BDFM".includes(r)) return "#ff6319"; // Orange
        if ("NQRW".includes(r)) return "#fccc0a"; // Yellow
        if ("JZ".includes(r))  return "#996633"; // Brown
        if ("L".includes(r))   return "#a7a9ac"; // Gray
        if ("G".includes(r))   return "#6cbe45"; // Light Green
        if ("S".includes(r))   return "#808183"; // Dark Gray
        return null;
    }
    ...
    function dp05ToDemog(p) {
        ...
        // DC uses 2018 schema (0082E), NYC uses 2019 schema (0077E). 
        // Math.max ignores the 'Some other race' overlapping variable.
        const whitePop = Math.max(+(p.DP05_0077E) || 0, +(p.DP05_0082E) || 0);
    ...
    async function loadDC() {
        const [acsGeo, stationGeo, routesGeo] = await Promise.all([
            d3.json(CFG.dc.acsFile),
            d3.json(CFG.dc.stationFile),
            d3.json(CFG.dc.routesFile).catch(() => ({ features: [] })),
        ]);
        ...
        const hoods = acsGeo.features.map(f => {
            // Fix winding order so D3 doesn't fill the outside of the polygon
        ...
        const routes = routesGeo.features.map(f => ({
            ...
            properties: {
                ...
                type: "route",
                transit_type: "metro",
                line_color: wmataColor(f.properties.NAME),
            },
        }));

        return {
            geoFeatures: [...hoods, ...routes, ...stations],
            demogRows: hoods.map(f => f.properties.demog),
        };
    }
    ...
    async function loadNYC() {
        ...
        const [stationGeo, routesGeo] = await Promise.all([
            d3.json(CFG.nyc.stationFile),
            d3.json(CFG.nyc.routesFile).catch(() => ({ features: [] })),
        ]);
        ...
        const stations = stationGeo.features.map(f => ({
            ...
            properties: {
                ...
                line_color: mtaColor(f.properties.daytime_routes),
            },
        }));
        ...
        const routes = routesGeo.features.map(f => ({
            ...
            properties: {
                ...
                type: "route",
                transit_type: "subway",
                line_color: mtaColor(f.properties.service),
            },
        }));
        ...
        return {
            geoFeatures: [...neighborhoods, ...routes, ...stations],
            demogRows,
            hasTractData,
        };
    }
    ...
    function buildBoroughChart(cfg, demogRows) {
        ...
        const legG = g.append("g").attr("transform", `translate(${cW - 120}, -40)`);
        const lgW = 80, lgH = 8;
    ...
    function drawBarChart({ svgId, groups, annotation, yLabel }) {
        ...
        g.selectAll(".bar-value-label").data(groups).join("text")
            .attr("class", "bar-value-label").attr("text-anchor", "start")
            .attr("x", d => midX(d) + 8).attr("y", d => y(d.mean) - 10)
        ...
        if (groups.length === 2) {
            ...
            if (gap > 0.02) {
                ...
                g.append("text").attr("class", "chart-annotation").attr("fill", "#f1c40f")
                    .attr("text-anchor", "start").attr("x", midX(hi) + 8).attr("y", y(hi.mean) - 28)
    ...
    function buildComparisonChart(nycRows, dcRows, nycHasTractData) {
        ...
        g.selectAll(".bar-value-label").data(groups).join("text")
            .attr("class", "bar-value-label").attr("text-anchor", "start")
            .attr("x", d => midX(d) + 8).attr("y", d => y(d.mean) - 8).attr("font-size", "0.7rem")
            .text(d => `${fmtKm(d.mean)}`);
